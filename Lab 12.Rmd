...
perm_mean <- function(perms = 1000, values, n1)
{
  ...
  for (i in c(1:perms))
  {
    ...
    rsamp <- sample(values, n1, replace = FALSE)
    other <- sample(values, length(values)-n1, replace = FALSE)
    # Step 3:
    # Compute the sample means for the two groups from 
    # step 2
    A <- mean(rsamp$lifeExp)
    B <- mean(other$lifeExp)
    # Step 4: 
...
correlation <- as.tibble(perm_mean(x=gdp_percap,y=life_exp))
```
